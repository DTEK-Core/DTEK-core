...
**Корень проблемы:** интеграция с email-провайдером (Resend, SendGrid, Supabase SMTP) в MVP не реализована. До S08-T003 invite-ссылка была доступна только через server-side fallback, а не через пользовательский интерфейс.
...
**Диагностика:** email-провайдер не интегрирован. Ссылка вида `${NEXT_PUBLIC_APP_URL}/invite/${token}` существует и работает — приглашённый может пройти по ней и зарегистрироваться. Проблема была в том, что ссылка не доставляется приглашённому автоматически.
...
1. Изменить тип возврата `sendInvitation` с `Promise<{ error?: string }>` на результат с `inviteUrl`, `email`, `role` и статусом результата
2. При успехе добавить к возврату `inviteUrl`:
   ...
   return { inviteUrl };
   ```
3. Если активное приглашение для email уже существует — не создавать новое, вернуть существующую ссылку
4. Если pending-приглашение истекло — вернуть статус `expired`; новую ссылку создавать только по явному действию пользователя
...
7. Для истёкшего приглашения показывать состояние «Истекло» и кнопку «Создать новое приглашение»

### Критерии готовности

- [x] После успешного создания приглашения диалог переходит в success-state с видимой ссылкой
- [x] Кнопка «Скопировать» копирует полный URL в буфер обмена
- [x] Кнопка «Пригласить ещё» сбрасывает форму
- [x] Повторное активное приглашение возвращает существующую ссылку без создания новой записи
- [x] Истёкшее pending-приглашение показывает «Истекло» и позволяет создать новую ссылку
- [x] Ссылка `/invite/{token}` открывается в браузере и показывает форму регистрации
- [x] TypeScript строгий режим: тип возврата `sendInvitation` обновлён
...
- [ ] Нет коммитов с `console.log` в production paths
