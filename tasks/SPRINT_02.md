...
- [ ] Новый пользователь может зарегистрироваться, создать организацию и пройти онбординг мастер без технических ошибок
- [ ] Существующий пользователь может войти, выйти, сбросить пароль
- [ ] Owner может пригласить участника по email; приглашённый может принять приглашение и войти
- [ ] Owner/Admin может видеть список участников и менять роли
- [ ] Настройки профиля и организации сохраняются корректно

### Технические

- [ ] Все маршруты `(app)/` защищены middleware: неавторизованный получает редирект на `/login`
- [ ] RLS работает: пользователь видит только данные своей организации (проверено в Supabase Studio)
- [ ] `npm run type-check` завершается с кодом 0 без ошибок TypeScript
- [ ] `npm run lint` завершается без ошибок ESLint
- [ ] `npm run build` завершается успешно
- [ ] CI-пайплайн (lint → type-check → build) проходит на ветке `feature/S02-*`
- [ ] Код влит в `develop` через PR; ветки `feature/S02-*` удалены
- [ ] `SUPABASE_SERVICE_ROLE_KEY` нигде не используется в клиентском коде; `.env.local` не закоммичен

### UX

- [ ] Все загрузки сопровождаются Skeleton-заглушками или disabled-кнопками; нет «пустых экранов»
- [ ] Ошибки форм отображаются inline, без перезагрузки страницы
- [ ] Toast-уведомления подтверждают успешные действия (сохранение, приглашение, смена роли)
- [ ] Цветовая схема, шрифты и компоненты соответствуют дизайн-прототипу `design/`
