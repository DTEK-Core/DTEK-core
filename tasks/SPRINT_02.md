...
- Исторически SMTP не был настроен; текущий Commercial MVP показывает manual invite link в `/users` (S08-T003, финализация S14-T003)
...
|---|---|---|---|
| Email provider для invitation не настроен | Высокая | Низкое | Текущий MVP использует поддержанный manual invite link; email delivery требует отдельного решения |
