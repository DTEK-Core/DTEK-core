...
authenticated QA и Pilot GO gates остаются pending. M-060–M-065 завершены как
Post-MVP architecture/security specifications и source decision; Sprint 15
documentation handoff готов. Runtime/UI/migrations не реализованы.
...
Форма приглашения: email + роль (analyst/admin/viewer). Запись в таблицу `invitations` и показ manual invite link для trusted delivery. Статус "Приглашён" в списке участников. Страница `/invite/{token}`: предложение регистрации или входа. Принятие приглашения — добавление в `profiles` с указанной ролью. Обработка истёкшего приглашения (7 дней). Запрет на приглашение с ролью `owner`.
