...
* **Sprint 15** — Post-MVP Connector Framework Foundation в работе; S15-T001–T007 завершены. Architecture/security contracts и Russian Market research shortlist готовы. S15-T007 принял решение `DEFER / KEEP CSV-XLSX IMPORT PATH`: source cards/demand scores остаются `PILOT DATA PENDING`, первый connector, runtime, UI и migrations не выбраны. Следующая задача — T008 Documentation Sync; decision пересматривается только после admission evidence.
