...
оставаться минимальной: `git status --porcelain=v1 --branch` и при необходимости
`git --no-pager log -1 --oneline`. Перед ответом проверить, что запущенные
Codex dev server, watcher или test runner завершены.
